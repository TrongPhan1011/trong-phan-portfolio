function CardExperience({ data, isLast = false, isFirst = false }) {
    const classFirst = isFirst ? 'bg-tp-orange' : 'bg-gray-700';
    return (
        <>
            <div className="flex w-full ">
                <div className="mr-4 flex  flex-col items-center">
                    <div>
                        <div className={'flex h-20 w-20 items-center justify-center rounded-full  p-2 ' + classFirst}>
                            <img src={data.logoCompanySource} alt="logo" className="h-12 w-12 object-contain" />
                        </div>
                    </div>
